...
BST_Node *BST_delete(BST_Node *root, int bar, double index)
{
	...
	if(root->key == key)
	{
		...
		{
			succ_node = find_successor(root->right);
			root->key = succ_node->key;
			root->freq = succ_node->freq;
			root->bar = succ_node->bar;
			root->index = succ_node->index;
...
//helper for comparing notes and changing
void shift_freq_help(BST_Node *root, int src_trck, int dst_trck)
{
	if(root == NULL)
		return;
	
	shift_freq_help(root->left, src_trck, dst_trck);
	
	shift_freq_help(root->right, src_trck, dst_trck);
	
	if (root->freq == note_freq[src_trck])
	{
		root->freq = note_freq[dst_trck];
	}
...
void BST_shiftFreq(BST_Node *root, char note_src[5], char note_dst[5])
{
	...
	if ((src_trck == -1) || (dst_trck == -1))
		return;
	
	
	shift_freq_help(root, src_trck, dst_trck);
...
//new linked list node for notes
BST_Node *newNoteNodeList(double key, double freq, int bar, double index)
{
...
//inserts new note in linked list
BST_Node *insertNoteList(double key, double freq, int bar, double index, BST_Node *head)
{
	BST_Node *new_note;
	new_note = newNoteNodeList(key, freq, bar, index);
	new_note->left = head;
	return new_note;
}
...
//traverses tree and inserts new note in list
BST_Node *BST_Harm_CRTLST(BST_Node *root, BST_Node *head, int semitones, double time_shift)
{
	double key;
	if(root == NULL)
		return head;

	head = BST_Harm_CRTLST(root->left, head, semitones, time_shift);

	for(int i = 0; i < 100; i++)
	{
		if((note_freq[i] == root->freq) && (0 <= i+semitones) && (i+semitones <= 99) && (0.0 <= root->index + time_shift) && (root->index + time_shift < 1.0))
		{
			key = (10.0*root->bar) + (root->index + time_shift);
			head = insertNoteList(key, note_freq[i+semitones], root->bar, root->index + time_shift, head);
		}
	}

	head = BST_Harm_CRTLST(root->right, head, semitones, time_shift);
	
	return head;
}

BST_Node *BST_HarmInsert(BST_Node *root, BST_Node *temp)
{
  	if(root == NULL)
		return temp;
	 
	if(fabs(root->key-temp->key) < 1e-15)
	{
		if(temp->index + 0.000001 < 1.000000)
		{
			temp->index = temp->index + 0.000001;
			temp->key = temp->key + 0.000001;
		}
		else
		{
			temp->index = temp->index - 0.000001;
		...
		return BST_HarmInsert(root, temp);
		
	}
	
	else if (root->key > temp->key)
	{
		root->left = BST_HarmInsert(root->left, temp);
	}
	
	else
	{
		root->right = BST_HarmInsert(root->right, temp);
	}
...
BST_Node *BST_harmonize(BST_Node *root, int semitones, double time_shift)
{
	...
	head = BST_Harm_CRTLST(root, head, semitones, time_shift);
